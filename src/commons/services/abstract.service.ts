...
export abstract class AbstractService<T extends Document> {
    ...
    public async insertOne(payload: AnyObject): Promise<T> {
        try{
            ...
            throw new BadRequestException(`Vérifiez vos informations, Il est possible qu'il y ait duplication de données comme numéro de téléphone ou email ou des données malformatées`);
        }
    }
